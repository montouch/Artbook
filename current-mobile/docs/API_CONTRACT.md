...
- `supabase/functions/support-worker/index.ts` is a fail-closed Edge Function scaffold for service-role support worker proof. It scans support cases/proof rows, records dry-run worker evidence in `artbook_audit_events`, and never calls delivery/message/payment/alert providers or moves money.
- `supabase/config.toml` disables Supabase JWT verification for `provider-webhook` and `support-worker`; both still require their own server-side secret checks before doing any work.
...
The migration is not live-applied yet because the connected Supabase account currently has no project visible and the Supabase CLI is not installed locally. When a Supabase project exists, apply with `supabase link --project-ref <project-ref>`, `supabase db push`, set `ARTBOOK_PROVIDER_WEBHOOK_SECRET`, then deploy `provider-webhook` and `support-worker`.
...
| `GET` | `/api/support/worker-plan` | Return the server-owned support worker plan for visible cases: delivery retry, SLA clock, provider callback review, care-audit gap and failure-alert owner lanes. It is review-only and says production needs a service-role worker. |
| `POST` | `/api/support/worker-runs` | Persist a dry-run worker result with queued actions and owner alerts. It can prove the support retry/SLA/callback/audit flow, but it never calls providers, sends alerts, closes cases, releases refunds, credits wallets, pays out providers or recognizes founder revenue. |
| Supabase Edge Function | `support-worker` | Hosted service-role scaffold for the same worker proof. `GET` returns a dry-run plan; `POST` writes per-account dry-run evidence to `artbook_audit_events`. It must be called by trusted scheduler/backend infrastructure with a secret key header, never from Android. |
